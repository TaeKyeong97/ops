...
<html lang="en" dir="ltr">
  <head>
    ...
    a{text-decoration:none;}
      ...
      .l-wrap .l-header{width:229px; height:79px; margin:0 auto; margin-bottom:50px; background:url('../images/main/common_lgo.png') no-repeat; background-position:-203px -27px;}
      ...
      .l-nave #naver-logo{width:185px; height:40px; background:url('../images/main/common_lgo.png') no-repeat; background-position:-13px -435px; display:block;}
...
<c:choose>
	<c:when test="${EM_id==null}">
		<div class="l-wrap">
		  <div class="l-header">
		    <a href="../ops/main" style="width:100%; height:100%;overflow:hidden; display:block;"></a>
		  </div>
		  <div class="login-menu" ondragstart='return false' onselectstart='return false'>
		  	<span class="logM1" onclick="logCk(1)" style="<c:choose><c:when test="${logCk.equals('0')||logCk=='0'}">${loginCk3}</c:when><c:otherwise>${loginCk1}</c:otherwise></c:choose>">아이디 찾기</span>
		  	<span class="logM2" onclick="logCk(2)" style="<c:choose><c:when test="${logCk.equals('1')||logCk=='1'}">${loginCk2}</c:when><c:otherwise>${loginCk1}</c:otherwise></c:choose>">비밀번호 찾기</span>  	
		  </div>
		  <form action="login_Pro.jsp?URL=${URL}&&kind=user" name="log1" method="post" class="log1" style="<c:choose><c:when test="${logCk.equals('0')||logCk=='0'}">display:block;</c:when><c:otherwise>display:none;</c:otherwise></c:choose>">  																						
			  <div class="fom-wrap">
				  <div class="l-login-con">
				    ...
				    <input type="text" name="phone" value="" placeholder="전화번호">
				  </div>
				  <div class="l-suumit-con">
				    <input class="l-submit" type="submit"value="아이디 찾기">
				  </div>
			  </div>
		  </form>
		  <form action="searchPassword?URL=${URL}&&kind=corp" name="log2" method="post" class="log2" style="<c:choose><c:when test="${logCk.equals('1')||logCk=='1'}">display:block;</c:when><c:otherwise>display:none;</c:otherwise></c:choose>">																			
			  <div class="fom-wrap">
				  <div class="l-login-con passId">
				    <input type="text" name="myId" value="" placeholder="아이디">
				  </div>
				  <div class="l-login-con passWord" style="display:none;">
				    <input type="text" name="phone" value="" placeholder="전화번호">
				  </div>
				  <div class="l-suumit-con passNext">
				    <input class="l-submit" type="button"value="다음" style="background:rgb(96, 129, 247)" onclick="ckPass()">
				  </div>
				  <div class="l-suumit-con passSubmit" style="display:none;">
				    <input class="l-submit" type="submit"value="비밀번호 찾기" style="background:rgb(96, 129, 247)">
				  </div>
			  </div>
			  <script>
			  	function ckPass(){
			  		var passId = document.getElementsByClassName("passId")[0];
			  		var passWord= document.getElementsByClassName("passWord")[0];
			  		var passSubmit= document.getElementsByClassName("passSubmit")[0];
			  		var passNext=  document.getElementsByClassName("passNext")[0];
		  ...
		  <div class="l-nave">
		    ...
		    <div class="sub-add">
		      <ul>
		        <li><a href="../JOIN/joinV1">회원가입</a></li>
		        <li>｜</li>
		        <li><a href="userFind?loginCk=0">아이디 찾기</a></li>
		        <li>｜</li>
		        <li><a href="userFind?loginCk=1">비밀번호 찾기</a></li>
		      </ul>
		    </div>
		  </div>
		</div>
...
<div class="l-wrap">
		  ...
		  <div class="login-menu">
		  	<span class="logM1" onclick="logCk(1)">아이디 찾기</span>
		  	<span class="logM2" onclick="logCk(2)">비밀번호 찾기</span>
		  </div>
			  <div class="fom-wrap">
				  <div class="l-login-con">
