...
<div id="join-wrap">
	...
	<div class="join-content">
      ...
      <div class="left join-input-con">
        <input type="text" class="liceName" name="liceName" id="code" placeholder="사업장명" style="width:35%; float:left;">
        <input type="text" class="liceNum" name="liceNum" id="code" placeholder="사업자등록번호(앞에서 6자리)" style="width:49%; margin-left:2%; float:left;">
        <button type="button" onClick="LicenCk()">등록 조회</button>
        <input type="hidden"name="lice" class="liceCode" value="">        
      </div>
      <div class="join-check lice"></div>
      ...
      <div class="left join-input-con">
        <input type="text" name="repName" id="repName" placeholder="대표자 명">
      </div>
      <div class="left join-input-con">
        <input type="text" name="repNum" id="repNum" placeholder="대표자 전화번호">
      </div>
      <div class="left join-input-con">
        <input type="text" name="typeOfBusiness" id="typeOfBusiness" placeholder="사업종류">
        <div id="wkplStylDvcd" style=" position: absolute; right: 0; line-height: 54px; width:78px; text-align:center; font-size:12px; ">(법인/개인)</div>
      </div>
      <div class="left join-input-con">
        <input type="text" name="joinNumber" id="joinNumber" placeholder="가입자수">
      </div>
      <div class="left join-input-con" style=" font-size: 12px; ">※ 가입자 수는 국민연금 공단에서 제공하는 정보이며 실제와 다를 수 있습니다.</div>
		</div>
	</div>
...
<script>
  function submits(form1){
    ...
    var lice=form1.lice.value;
    
    if(EM_id==""&&EM_pw==""&&EM_phon==""&&lice==""){
      ...
      document.getElementsByClassName("lice")[0].innerHTML="사업자 조회가 필요합니다.";
      form1.id.focus();
      return false;
    }else if(EM_id==""&&EM_pw==""){
      document.getElementsByClassName("checkID")[0].innerHTML="필수정보입니다.";
      ...
      document.getElementsByClassName("lice")[0].innerHTML="";
      form1.id.focus();
      return false;
    }else if(EM_pw==""&&EM_phon==""){
    ...
    writeform.action="joinPro2?myId="+myId;
    form1.submit();
  }
</script>
...
<script>
	...
	function joinIdCk(writeform){
		 ...
		 var ids=id.value;
		 window.open("overCK2?EM_id="+ids, "네이버새창", "width=370, height=300, toolbar=no, menubar=no, scrollbars=no, resizable=yes" );  
	}
	function okes(ok){
		var id=document.getElementsByClassName("EM_id")[0];
	...
	function reCk(){
		...
		location.href="overCK2?EM_id="+ok.value;
	}
	//등록번호 체크
	function LicenCk(writeform){
		var liceName=document.getElementsByClassName('liceName')[0];
		var liceNum=document.getElementsByClassName('liceNum')[0];
		liceName=liceName.value;
		liceNum=liceNum.value;
		window.open("LicenceseSearch?liceName="+liceName+"&&liceNum="+liceNum, "네이버새창", "width=370, height=300, toolbar=no, menubar=no, scrollbars=no, resizable=yes" );  
		 
	}
	function emokes(ok,company,corpfom,vldtVlKrnNm,jnngpCnt){
		var id=document.getElementsByClassName("liceName")[0];
		var cks=document.getElementsByClassName("liceCode")[0];
		id.value=ok;
		cks.value='1';
		alert("사업자 형태가 확인 되었습니다. ("+corpfom+")");
		$("#wkplStylDvcd").html("<span style='font-weight:bold; color:#3b60f5;'>("+corpfom+")</span>");
		$("#typeOfBusiness").val(vldtVlKrnNm);
		$("#joinNumber").val(jnngpCnt);
	}
