...
<html lang="ko" dir="ltr">
  ...
  <div class="sche-popup-wrap" style="z-index:0; top:100%; opacity:0;">
    ...
    <div class="sche-date-wrap">
      ...
      <textarea name="content" id="contenta" rows="9" cols="97"></textarea>
    </div>
  </div>
<script>
  ...
  */
  ...
  function scheAdd(){
    ...
    $.ajax({
		...
		method:'post',
		dataType:'html',
		data:{'no':${param.no},startMonth:startDate,endMonth:endDate,startDate:$(".chepopup-startDate").val(),endDate:$(".chepopup-endDate").val(),ampm:$(".ampm").val(),contenta:$("#contenta").val()},
		seccess:function(response){				
		}
	});
    //새로 고침    
  ...
  *
  */
  var arrayList = [<c:forEach var="sbList" items="${sbList}" varStatus="status">[${sbList.startMonth},${sbList.endMonth},${sbList.no},'${sbList.startDate}','${sbList.endDate}','${sbList.ampm}','${sbList.content}'],</c:forEach>];
	...
	for(var schListX=0;schListX<scheListIndex;schListX++){
		schHtml="<div class=\"schedule-row\">"
				      +"<div class=\"schbar schWNum-"+Number(schListX+1)+"\" style=\"width:"+Number(schIndexArray[Number(arrayList[schListX][1])]-schIndexArray[Number(arrayList[schListX][0]-1)])+"px;  left:"+schIndexArray[Number(arrayList[schListX][0]-1)]+"px;\">"
				        ...
				        +"<div class='schcon' style='position: absolute; display:none; z-index: 500; background: #28293a; border: 1px solid #8a27c0; padding: 18px 30px; top: 0; right:-143px;'>"+arrayList[schListX][6]+"</div>"
				      +"</div>"
				    +"</div>"
					...
					+"<textarea class='cont-"+Number(schListX+1)+"' style='display:none;' name='contenta'>"+arrayList[schListX][6]+"</textarea>"
	    $(".schedule-wrap").append(schHtml);	    
	}
	/*
	
	$(".schbar").hover(function() {
		$(event.target).children('div.schcon').css("display","block");
	}, function() {
		$(event.target).children('div.schcon').css("display","none");
	});*/
</script>
  </body>
</html>
